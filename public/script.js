...
// Enhanced script.js مع نظام الإطارات المُصحح
// ===== إعدادات الإطارات =====
const FRAME_SYSTEM = {
    frames: {
...
function openAppStore() {
    // إغلاق القائمة الرئيسية
    if (document.getElementById('mainMenuModal')) {
        document.getElementById('mainMenuModal').style.display = 'none';
    }
    
    // إزالة المودال السابق إن وُجد
    const existingModal = document.getElementById('appStoreModal');
    if (existingModal) {
            ...
            <div class="store-content">
                <div class="store-header">
                    <div class="user-coins">
                        ...
                        <span id="userCoinsDisplay">${currentUser.coins || 2000}</span>
                        <span>نقطة</span>
                    </div>
                ...
                <div id="storeTabContent" class="store-tab-content">
                    <p style="text-align: center; padding: 20px;">جاري تحميل المحتوى...</p>
                </div>
            </div>
        </div>
    `;
...
// ===== الحصول على نص التبويب =====
function getTabText(tabName) {
    switch(tabName) {
        case 'frames': return 'الإطارات';
        case 'decorations': return 'الزخارف';
        case 'my-items': return 'مشترياتي';
        default: return '';
    }
}
...
function createFramesTabHTML() {
    ...
    // إطارات الإدارة
    if (currentUser.rank === 'admin' || currentUser.isOwner) {
        html += `
            <div class="frame-category">
    ...
    return `
        <div class="frame-card ${frame.rarity}" data-frame-id="${frame.id}">
            <div class="frame-preview">
                <div class="frame-image ${frame.rarity}-frame animated-frame" data-animation="${frame.animation}">
...
function buyFrame(frameId) {
    ...
    if (!frame) {
        showToast('❌ إطار غير موجود!', 'error');
        return;
    }
    ...
    if (currentUser.ownedFrames.includes(frameId)) {
        showToast('❌ تملك هذا الإطار بالفعل!', 'error');
        return;
    }
...
function activateFrame(frameId) {
    ...
    if (!currentUser.ownedFrames.includes(frameId) && !currentUser.isOwner) {
...
function updateUserFrame() {
    ...
    if (userAvatar && userAvatar.parentElement) {
        userAvatar.parentElement.className = `user-avatar-mini frame-${frame.rarity}`;
        userAvatar.parentElement.setAttribute('data-animation', frame.animation);
    }
}
...
function createMyItemsTabHTML() {
    if (!currentUser.ownedFrames || currentUser.ownedFrames.length === 0) {
        return `
